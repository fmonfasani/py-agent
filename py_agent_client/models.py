...
class RouteRequest(BaseModel):
    ...
    context: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional context data for the request"
    )
    ...
    max_cost: Optional[float] = Field(
        default=None, ge=0, description="Maximum cost limit for this request"
    )
    quality_threshold: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Minimum quality score required"
    )
    timeout: Optional[float] = Field(
        default=30.0, gt=0, description="Request timeout in seconds"
    )
    max_tokens: Optional[int] = Field(
        default=None, gt=0, description="Maximum tokens in response"
    )
    ...
    excluded_providers: Optional[List[ProviderName]] = Field(
        default=None, description="Providers to exclude from routing"
    )
    session_id: Optional[str] = Field(
        default=None, description="Session ID for context management"
    )
...
class RouteResponse(BaseModel):
    ...
    quality_score: float = Field(
        ..., ge=0.0, le=1.0, description="Estimated quality score of the response"
    )
...
class ProviderConfig(BaseModel):
    ...
    max_requests_per_minute: Optional[int] = Field(
        default=None, gt=0, description="Rate limit for requests per minute"
    )
    ...
    models: List[ModelInfo] = Field(
        default_factory=list, description="Available models for this provider"
    )
...
class BudgetConfig(BaseModel):
    ...
    daily_limit: float = Field(
        default=100.0, ge=0, description="Daily spending limit in USD"
    )
    monthly_limit: float = Field(
        default=1000.0, ge=0, description="Monthly spending limit in USD"
    )
    ...
    auto_scale_budget: bool = Field(
        default=False, description="Automatically scale budget based on usage patterns"
    )
...
class ContextConfig(BaseModel):
    ...
    max_context_size: int = Field(
        default=4096, gt=0, description="Maximum context size in tokens"
    )
    context_retention_hours: int = Field(
        default=24, gt=0, description="How long to retain context in hours"
    )
    enable_compression: bool = Field(
        default=True, description="Enable context compression"
    )
    enable_summarization: bool = Field(
        default=True, description="Enable context summarization"
    )
...
class TelemetryConfig(BaseModel):
    ...
    batch_size: int = Field(
        default=100, gt=0, description="Batch size for telemetry uploads"
    )
    flush_interval: int = Field(
        default=300, gt=0, description="Flush interval in seconds"
    )
    include_prompts: bool = Field(
        default=False, description="Include prompts in telemetry (privacy setting)"
    )
    include_responses: bool = Field(
        default=False, description="Include responses in telemetry (privacy setting)"
    )
...
class AgentConfig(BaseModel):
    ...
    context: ContextConfig = Field(
        default_factory=ContextConfig, description="Context configuration"
    )
    telemetry: TelemetryConfig = Field(
        default_factory=TelemetryConfig, description="Telemetry configuration"
    )
    default_optimization: OptimizationMode = Field(
        default=OptimizationMode.BALANCED, description="Default optimization mode"
    )
    enable_fallbacks: bool = Field(
        default=True, description="Enable provider fallbacks"
    )
...
class UsageStats(BaseModel):
    ...
    provider_distribution: Dict[str, int] = Field(
        default_factory=dict, description="Request distribution by provider"
    )
    model_distribution: Dict[str, int] = Field(
        default_factory=dict, description="Request distribution by model"
    )
    optimization_distribution: Dict[str, int] = Field(
        default_factory=dict, description="Request distribution by optimization mode"
    )
    daily_costs: List[float] = Field(
        default_factory=list, description="Daily cost history"
    )
    quality_trend: List[float] = Field(
        default_factory=list, description="Quality score trend"
    )
...
class ErrorReport(BaseModel):
    ...
    request_context: Dict[str, Any] = Field(
        default_factory=dict, description="Context when error occurred"
    )
    stack_trace: Optional[str] = Field(
        default=None, description="Stack trace if available"
    )
    resolution: Optional[str] = Field(
        default=None, description="How the error was resolved"
    )
